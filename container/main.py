"""
FILE:    main.py  (v2.0 — production container version)
...
PURPOSE: AI processing pipeline + browser-accessible annotated video stream.
         This file has ZERO direct hardware access — all hardware is
         controlled through bridge.py via HTTP REST commands.

HOW IT WORKS:
  1. Connects to bridge.py and pulls raw MJPEG frames
       ← GET http://localhost:5000/stream
  2. Runs the full AI pipeline on each frame:
       - Face detection        : OpenCV Haar Cascade
       - Face verification     : DeepFace FaceNet (cosine similarity)
       - Emotion analysis      : DeepFace Mini-Xception
       - Drowsiness detection  : custom Keras CNN (eye_state_model.h5)
  3. Draws ALL OpenCV overlays onto the frame (bounding box, state label,
     emotion, drowsy badge, status bar)
  4. Serves the annotated frame as MJPEG in a browser
       → GET http://0.0.0.0:5001/         (HTML page)
       → GET http://0.0.0.0:5001/stream   (raw MJPEG)
  5. POSTs the AI result as JSON to bridge.py for hardware actuation
       → POST http://localhost:5000/command
  6. Sends Firebase photo upload and ThingSpeak writes via connect.py
     (non-blocking, background ThreadPoolExecutor)

FIXES vs v1.0 (main.py bare-metal):
  - No cv2.imshow() — display is browser-based MJPEG on port 5001
  - No sleep() on hot path — throttling via frame counter
  - Firebase initialized once via connect.py guard
  - ThingSpeak rate-limited (1 write / 15s per field)
  - All cloud I/O is fire-and-forget (thread pool)
  - IR gate checked once at startup, not every frame

DEPENDENCIES (inside container): opencv-python, deepface, tensorflow,
                                  flask, requests, numpy
USAGE:
  Terminal 1 (Pi host):      python3 bridge.py
  Terminal 2 (Pi container): docker run ... driver-monitor:pi python3 main.py
"""
